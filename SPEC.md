...
**Export-side: the companion enum is only emitted if actually used, by default** -- if nothing in the compiled data uses `@Domain` for a given domain, no companion enum appears in the output, even if that domain declared a width. See §8.5 and §14.7 for `--emit-all-domains`, which overrides this (force-emitting an indexed enum with nothing referencing it in the data itself, for hand-written dispatch code).
...
**By default, every export target suppresses a domain's output entirely if nothing in the compiled data references it, even if that domain declared a width.** `gather_domains_used()` collects only domains actually referenced by some field after composition flattening, and each of `export_6502.py`, `export_z80.py`, and `export_68000.py` has a `continue`-on-not-in-`used` line in its own `gather_domain_info()`, structurally identical across all three.

**`--emit-all-domains` overrides this on every target, one shared boolean, no per-domain selection list.** Hand-written low-level code (an assembly dispatch table, a 68000 C jump table) can want a domain's compact constants available without GDDL ever storing a value from that domain in any compiled struct; this flag makes that possible everywhere. Confirmed directly against a real fixture on all four targets: a width-declared, zero-reference domain produces no output at all by default, and its full target-appropriate representation with the flag on. It is not literally the same mechanism on every target, though, and describing it that way would be wrong:

- **On C++**, a referenced domain already always gets its default logical-ID enum; only the `@`-gated `_Indexed` companion needs the flag to appear when otherwise unused (§14.7). This is about a *second form* of an *already-emitted* domain.
- **On 6502, Z80, and 68000**, `Domain` and `@Domain` are indistinguishable, there is only ever one representation, always the compact/indexed form. This is about the *entire domain* being absent, not a missing companion of something already present.

When the flag is on, every domain that declared a width gets its target-appropriate compact representation emitted, regardless of whether anything in the compiled data references it. When off (the default), current per-target "only emit if used" behavior is unchanged. A domain with no declared width is unaffected either way, on every target, since there's nothing for it to force-emit.
...
Both 6502 and Z80 have string field support implemented and validated against real assemblers (confirmed directly: real `!text "..."`/`db "...", 0, ...` output on both, assembled and read back correctly). This rule was originally written forward-looking, before either target had it; it's now describing what both already do, not a plan for what they will.
...
|---|---|---|---|
| C++ | Yes (default) | Yes (`--layout=aos-linear`) | Yes (`--layout=soa`) |
| 6502 | Yes (always; no alternative exists, and none is planned — see below) | Not implemented | Yes (`--layout=soa`) |
| Z80 | See note below | Yes (baseline, always) | Yes (`--layout=soa`, all three output paths -- see note below on scope differences) |
...
**Z80's three output paths do not all support the same SoA field-type scope, and this is deliberate, not an oversight.** SjASMPlus and `z88dk` assembly mode (§16.1) hand-write the index-to-array-offset multiply themselves (§16, the shift-add-doubling scheme), and that renderer only exists for a power-of-two element stride -- so both assembly dialects' `--layout=soa` explicitly rejects `string N`-typed and array-typed (§21) leaf fields, whose strides are usually not powers of two, with a clear compile-time error rather than emitting something wrong. `z88dk` C mode (§16.3) is different: it never hand-writes indexing at all, C mode's whole design leaves every index computation to `zsdcc`, which strength-reduces `index * stride` for any constant stride (the same reasoning that already lets AoS mode's `{Type}_Find` support every struct size, §16.1, unlike `sccz80`'s enumerated-multiplier cliff). So C mode's `--layout=soa` has no such restriction: scalar, identifier-typed, `string N`-typed, and array-typed leaf fields are all supported, verified with a real `zsdcc` compile+link (HANDOFF.md).
...
**Why C++ historically had no linear-list alternative, despite nothing preventing one:** unlike 6502, this was never a hardware constraint — modern hardware multiplication is effectively free. C++ chose named-per-instance globals plus a pointer-holding registry (§14.2) specifically so calling code can write `Item_Instances::Sword.power` directly, which a pure-array design gives up in exchange for guaranteed contiguity. **That tradeoff is no longer a forced choice: C++ supports a third, opt-in layout** (`--layout=aos-linear`, alongside the existing `aos` and `soa` values on the same flag, per §13.6's "one flag, one job" precedent) — instances stored directly in one `std::array<T, N>`, matching what 68000 already proves out for the underlying pattern, with `Find()` still returning `const Item*` (computed as `&All[i]`, not a lookup-table pointer) so existing pointer-list calling code needs no changes beyond recompiling against the new layout. Confirmed with real `g++17` compilation and execution, single-header and split modes both, contiguity checked directly via pointer arithmetic rather than assumed from `std::array`'s documentation.
...
**A domain that declares a width gets a companion enum, generated only if something actually uses `@Domain` somewhere in the compiled data, unless `--emit-all-domains` is on (§8.5):**
...
- **Pool table (§22.4, format_version 2):** for every pool present in this file — pool name, the type it reserves records of, that type's schema hash and record size, and the pool's own declared count. Carries no offset at all: a pool's own records are never written anywhere in this file (§22 pools are always uninitialized), so the reader is responsible for allocating its own storage. See export_binary.py's own docstring for the exact byte layout, the authoritative definition per this section's own established convention.
...
**Status: declaration and registry semantics implemented and verified. Export: implemented and verified on all five targets.** C++ (single-header and split mode); 6502 (SoA only, AoS deferred); Z80 (SoA only on both assembly dialects, z88dk C mode and AoS deferred); 68000 (both layouts, full field-type support); standalone binary (format_version 2, see below). This completes pools' stage 3 (export) in full.
...
Fields wider than a byte get the same Lo/Hi split convention named instances' own SoA columns already use (SS10.2) -- `EntityPool_tag_Lo`/`EntityPool_tag_Hi`, each a `count`-byte region -- so game code written against a named instance's SoA field indexes a pool's field identically (`LDA EntityPool_tag_Lo,X`). **`string N` and array-typed leaf fields are rejected**, for the identical reason named-instance SoA export already rejects them on this target: indexing pool slot i's field would need `base + i * stride` for a `stride` not guaranteed a power of two, a real multiply this target has no general answer for yet -- a pool offering that field anyway would silently produce something that looks like free SoA indexing but isn't. Verified end to end against all three real assemblers/emulators: assembled clean, confirmed zero added output bytes, and a real py65-executed write into one pool slot's field (via indexed absolute addressing) read back correctly with every other slot confirmed untouched.
...
**Standalone binary (implemented and verified) -- format_version bumped 1 -> 2:** this is the one target this whole section's "genuinely uninitialized" design goal fits most literally, since the compiled artifact IS the runtime data (§17's own framing: "readable by third-party software... at runtime"), not source compiled elsewhere. The global header gains a `pool_count` (u32); a new POOL TABLE section (name, type_name, schema_hash, record_size, record_count -- no offset field at all) follows the existing per-type table. A pool table entry writes **zero bytes anywhere else in the file** for its own records -- the runtime loader reading this format is expected to allocate `record_size * record_count` bytes of its own memory for the pool the moment it reads the entry, the same "genuinely free" property every other target's pool storage already has, but here it's not an implementation detail of some downstream compiler -- it's the format's own documented contract with its reader. `schema_hash`/`record_size` are computed via the exact same functions a real type-table entry's own are (`compute_schema_hash`/`compute_record_size`), confirmed directly to match byte-for-byte when a pool and a same-shaped type both appear in one file. The `.gddlmeta.json` manifest gained a parallel `pools` array (name, type, schema_hash, record_size, count, full field list) alongside the existing `types` array. A format_version bump is a real, deliberate breaking change (§17.6's own "keeping that computation identical across implementations is a real, ongoing correctness obligation"): a reader built against format_version 1 has no `pool_count` field to read and must fail cleanly rather than misparse the file -- confirmed by updating this project's own independent from-scratch reader (`tests/export_binary_test/independent_reader.py`, deliberately never importing the writer's own code) to require version 2 and parse the new pool table, then re-verifying every existing check against it still passes.
