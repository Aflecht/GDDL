...
def render_pools_z88dk_asm(pools: list, pool_alloc) -> list:
    """§22.4: identical shape to export_z80_sjasmplus.render_pools_
    sjasmplus -- both assembly dialects share `equ` syntax on this
    target (confirmed directly against the real z88dk-z80asm binary,
    not assumed from SjASMPlus's own confirmed behavior)."""
    lines = []
    for pool in pools:
    ...
    if layout == "soa":
        ...
        for t in types:
            ...
            for path, type_tokens, values in gather_soa_columns(t):
                ...
                lines.append("")
        lines.extend(render_pools_z88dk_asm(pools, pool_alloc))
        return "\n".join(lines)
