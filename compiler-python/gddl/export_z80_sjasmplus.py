...
def render_pools_sjasmplus(pools: list, pool_alloc) -> list:
    """§22.4: one `equ` constant per pool leaf field region -- never a
    data directive (`db`/`dw`/`defs`). Confirmed directly against the
    real SjASMPlus binary (v1.24.0): a bare `Label: equ expression`
    costs zero output bytes under --raw, mirroring the identical
    finding already confirmed on 6502 (ACME/64tass/KickAssembler all
    agree on this). No Lo/Hi split -- unlike 6502, a u16 pool field is
    ONE contiguous `2*count`-byte region here (this target's own richer
    register set already makes `add hl,hl` indexing free without
    needing byte-plane splitting, matching how named-instance SoA u16
    columns already work on this target, export_z80.py's own module
    docstring)."""
    lines = []
    for pool in pools:
        ...
        for path, _type_tokens in pool.leaves:
            region = regions[path]
            lines.append(f"{pool.name}_{path} equ ${region.addr:04x}")
        lines.append("")
    return lines


def render_sjasmplus(domains: list, types: list, pointer_table: bool = True,
                     find_macro: bool = False, reg=None, layout: str = "aos",
                     pools: list = None, pool_alloc=None) -> str:
    from .export_z80 import type_sizeof, gather_soa_columns, flatten_array_ir_value
    pools = pools or []
            ...
            for path, type_tokens, values in gather_soa_columns(t):
                ...
                if _try_parse_array_type(type_tokens.strip()) is not None:
                    raise ExportZ80Error(
                        ...
                        "supported in --layout=soa -- matching this target's "
                        "own SoA string-field gap just above (and 6502's "
                        "identical precedent), arrays are AoS-only for now; "
                        "use --layout aos instead")
                ...
                lines.append("")
        lines.extend(render_pools_sjasmplus(pools, pool_alloc))
        return "\n".join(lines)
