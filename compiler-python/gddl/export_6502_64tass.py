...
def render_pools_64tass(pools: list, pool_alloc: PoolAllocation) -> list:
    """§22.4: one plain constant assignment per pool leaf field region --
    same "never a data directive, always a bare constant" reasoning as
    export_6502_acme.render_pools_acme, confirmed independently against
    the real 64tass binary (same `Label = $addr` syntax, same
    zero-output-bytes behavior under --nostart)."""
    lines = []
    for pool in pools:
            ...
            for path, type_tokens, values in gather_soa_columns(t):
                ...
                if width == 1:
                    ...
                    for r in rendered:
                        lines.append(f"\t.byte >{r}")
            lines.append("")
        lines.extend(render_pools_64tass(pools, pool_alloc))
        return "\n".join(lines)
                ...
                if array_info is not None:
                    ...
                    for i, v in enumerate(flat):
                        ...
                        if elem_n is not None:
                            lines.extend(render_string_leaf_64tass(v, elem_n, elem_path))
                        else:
