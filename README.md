...
GDDL lets you define your game's structured data once — item stats, character definitions, ability tables, loot tables, anything with a fixed shape you'd otherwise hand-write as a struct or a spreadsheet — and compile it into whatever your game actually needs: real C++ structs, retro 6502/Z80/68000 assembly or C, or a portable binary file your game loads at runtime.
...
- [`docs/export-targets/cpp.md`](docs/export-targets/cpp.md) — compile GDDL data directly into your game.
- [`docs/export-targets/binary-format.md`](docs/export-targets/binary-format.md) — load data at runtime instead: DLC, mods, or just skipping a full rebuild for every data tweak.
- [`examples/cpp-load-binary-data/`](examples/cpp-load-binary-data/) — a minimal, working example of both.
...
- [`SPEC.md`](SPEC.md) — the complete, authoritative reference.
...
| [`docs/`](docs/) | Practical, target-specific guides — how to actually use each export target. |
| [`examples/`](examples/) | Minimal, working example projects. Start here if you learn by reading code. |
| [`compiler-python/`](compiler-python/) | The reference implementation (Python). This is where GDDL is actually built — only relevant if you're contributing to the compiler itself, not just using it. |
| [`compiler-cpp/`](compiler-cpp/) | An embeddable C++ implementation of the compiler, for projects that want to compile `.gddl` files as part of their own build. In development. |
